...
// ================= OBTENER DATOS =================
$stmt = $conn->query("SELECT * FROM paginaweb_hn_diaria WHERE id = 1");
...
/* =====================================================
   AFINADO ACORDEÓN – ESTILO MULTI‑X (LA DIARIA)
   ===================================================== */
...
/* ===== TEXTO INTERNO (SEMIBOLD MULTI‑X) ===== */
.accordion-content,
.accordion-content p,
...
/* ===== IMAGEN + TEXTO (MISMO LAYOUT MULTI‑X) ===== */
.linea-juego {
  background: transparent;
  box-shadow: none;
  padding: 0;
...
/* LOGO LA DIARIA – PROTAGONISTA */
.accordion .img-diaria {
  width: 190px;   /* mismo tamaño que Multi‑X */
  max-width: 100%;
  height: auto;
  margin-bottom: 20px;
}
...
/* ===== SUB‑ACORDEONES (MISMO ESTILO MULTI‑X) ===== */
.sub-accordion-header {
  background: #029247;
  font-size: 16px;
  font-weight: 600;
  ...
  <div class="popup-content">
    <div class="popup-image-wrapper">
      ...
      $imagen = (!empty($popup['imagen_url'])) 
                ? $popup['imagen_url'] 
                : 'ImagesSV/large-BANNERPOPUPLADIARIA (1) 1.jpg';
...
<body>
  ...
  <div class="top">
    <div class="top-content">
      <img src="<?= !empty($config['logo']) ? $config['logo'] : '/ImagesSV/logo-12- LA DIARIA.png' ?>" alt="Logo Diaria">

      <div class="ganador-box">
        <div class="ganador">ÚLTIMO NÚMERO GANADOR</div>

        <div class="nums fila-resultados">
  <span class="num"><?= htmlspecialchars($digito1) ?></span>
  <span class="num"><?= htmlspecialchars($digito2) ?></span>

  <span class="num mas1">
    <img src="/ImagesSV/logo-13-MÁS 1.png" alt="Más 1">
  </span>

  <span class="num extra"><?= htmlspecialchars($extra) ?></span>
...
<div class="menu">
  ...
  <a href="https://pantallasreatilcachnprod.azurewebsites.net/index.php?pag=num_sorteados_mes">
    TOP NÚMEROS GANADORES
  </a>

  <a href="/ImagesSV/Guia de los sueños.png" download>
    DESCARGAR LA GUÍA DE LOS SUEÑOS
  </a>
  ...
  <div class="sorteo">
    ...
    <div class="fila-resultados">
      ...
      <img src="/ImagesSV/logo-13-MÁS 1.png" class="num mas1" alt="Más 1">
...
<div class="accordion">
  ...
  <div class="info-juego">
    ...
    <div class="img-container">
        <img src="/ImagesSV/logo-12- LA DIARIA.png" class="img-diaria" alt="Imagen Diaria">
    </div>
...
<div class="reglamento">
  <a href="/ImagesSV/documentos/Reglamento_Diaria.pdf" target="_blank">
...
<script>
function pintarDiaria(pref, data){
  if(!data || !data.diaria){
    document.getElementById(pref+'_1').innerText = '0';
    document.getElementById(pref+'_2').innerText = '0';
    document.getElementById(pref+'_3').innerText = '0';
    return;
  }

  const num = String(data.diaria.par1).padStart(2,'0');

  document.getElementById(pref+'_1').innerText = num[0];
  document.getElementById(pref+'_2').innerText = num[1];

  // ✅ MÁS 1
  if(data.mas1 && data.mas1.par1){
    document.getElementById(pref+'_3').innerText =
      String(data.mas1.par1).padStart(1,'0');
  }else{
    document.getElementById(pref+'_3').innerText = '0';
  }
}

function cargarResultadosDiaria(fecha){
  fetch(`/api/resultados_diaria_por_fecha.php?fecha=${fecha}`)
    .then(r=>r.json())
    .then(d=>{
      pintarDiaria('num11', d['11']);
      pintarDiaria('num15', d['15']);
      pintarDiaria('num21', d['21']);
    });
}
</script>
