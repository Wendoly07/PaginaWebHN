...
// Estado del ciclo
let siguienteTipo = null; // 'sabado' o 'miercoles'

// Obtener próximo objetivo REAL en secuencia fija
function obtenerSiguienteFecha() {

    const ahora = new Date();
    const dia = ahora.getDay(); // 0=Dom, 3=Mié, 6=Sáb

    let objetivo = new Date(ahora);
    objetivo.setHours(21,0,0,0);

    function diasHasta(target){
        let d = target - dia;
        if (d < 0) d += 7;
        return d;
    }

    // PRIMERA VEZ: decidir arranque
    if (!siguienteTipo) {

        const distSab = diasHasta(6);
        const distMie = diasHasta(3);

        // Elegimos el más cercano SOLO la primera vez
        siguienteTipo = (distSab <= distMie) ? 'sabado' : 'miercoles';
    }

    let diasAgregar;

    if (siguienteTipo === 'sabado') {

        diasAgregar = diasHasta(6);

        // Si hoy es sábado pero ya pasó las 9pm
        if (diasAgregar === 0 && ahora >= objetivo) {
            diasAgregar = 7;
        }

    } else {

        diasAgregar = diasHasta(3);

        // Si hoy es miércoles pero ya pasó las 9pm
        if (diasAgregar === 0 && ahora >= objetivo) {
            diasAgregar = 7;
        }
    }

    objetivo.setDate(ahora.getDate() + diasAgregar);

    return objetivo;
}

// Inicial
let fechaObjetivo = obtenerSiguienteFecha().getTime();
...
setInterval(() => {
    ...
    if (diff <= 0) {
        ...
        // 🔁 CAMBIO DE CICLO
        siguienteTipo = (siguienteTipo === 'sabado') ? 'miercoles' : 'sabado';

        fechaObjetivo = obtenerSiguienteFecha().getTime();
        return;
    }

    const h = pad(Math.floor((diff % day) / hour));
    const m = pad(Math.floor((diff % hour) / minute));
    const s = pad(Math.floor((diff % minute) / second));
